...
<div class="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    <h2 class="text-2xl font-bold text-slate-900 sm:text-3xl">Selamat datang, {{ auth()->user()->name }}</h2>
    <p class="mt-3 max-w-4xl text-slate-600">Kelola struktur indikator, target tahunan, realisasi, dokumen pendukung, berita, dan publikasi dari satu panel.</p>
</div>
...
<div class="grid gap-6 xl:grid-cols-2">
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
            ...
            <a href="{{ route('admin.beritas.index') }}" class="text-sm font-semibold text-teal-700 hover:underline">Lihat semua</a>
        </div>
        <div class="divide-y divide-slate-100">
            @forelse($beritas as $berita)
                <div class="px-6 py-4">
                    <p class="font-semibold text-slate-800">{{ $berita->judul }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ $berita->created_at->format('d M Y') }} · {{ $berita->penulis ?: 'Tanpa penulis' }}</p>
                </div>
            @empty
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4">
            ...
            <a href="{{ route('admin.publikasis.index') }}" class="text-sm font-semibold text-teal-700 hover:underline">Lihat semua</a>
        </div>
        <div class="divide-y divide-slate-100">
            @forelse($publikasis as $publikasi)
                <div class="px-6 py-4">
                    <p class="font-semibold text-slate-800">{{ $publikasi->judul }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ $publikasi->created_at->format('d M Y') }} · {{ $publikasi->penulis ?: 'Tanpa penulis' }}</p>
                </div>
            @empty
